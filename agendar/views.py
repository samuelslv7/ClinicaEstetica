...
def horario_cancelar(request: HttpRequest):
    ...
    contexto = {
        "horarios": HorarioModel.objects.filter(livre=True).filter(
...
def agendamento_realizar(request: HttpRequest):
    if request.method == "POST":
        ...
        if formulario.is_valid():
            ...
            messages.error(
                request,
                "Erro ao realizar o agendamento. Verifique os dados informados.",
            )
...
def agendamento_cancelar(request: HttpRequest):
    ...
    contexto = {
        "horarios": HorarioModel.objects.filter(livre=False)
...
def agenda_horarios(request):
    ...
    horariosSemana = (
        HorarioModel.objects.filter(data__range=[segunda, segunda + timedelta(days=6)])
        .select_related("agendamento__cliente")
        .order_by("horario")
    )
